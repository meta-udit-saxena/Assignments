 ...
 * @return msg - message 
 */
SinglyLinkedList.prototype.removeByValue = function (data) {
    var start = this.head;
    var msg;
    if (start === null) {
        ...
        var previousNode;
        while (count <= maxLength && start.data != data) {
            previousNode = start;
            start = start.next;
            count++;
        }
 ...
 * @return msg - message
 */
SinglyLinkedList.prototype.removeByPosition = function (position) {
    var start = this.head;
    var msg;
    var maxLength = this.length;
    var count = 1;
 ...
 * @return msg - message
 */
SinglyLinkedList.prototype.searchData = function (data) {
    var start = this.head;
    var msg;
    if (start === null) {
        msg = "Empty Linked List";
        ...
        while (count <= length && start.data != data) {
            start = start.next;
            count++;
        }
        if (count > length) {
...
function removeDataByValue() {
    ...
    var flag = checkInput(value);
    if (flag === true) {
        displayMessage(obj.removeByValue(value));
        displayLinkedList();
    } else {
        displayMessage("Enter Value");
    }
